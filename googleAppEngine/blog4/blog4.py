...
class Handler(webapp2.RequestHandler):
	...
	def render_json(self, d):
		...
		self.write(json_txt)
	def mc_set(self, key,val):
		memcache.set(key,(val, datetime.utcnow()))
		return None
	def mc_get(self, key):
		if memcache.get(key):
			return (memcache.get(key)[0], (datetime.utcnow() - memcache.get(key)[1]).total_seconds())
		return (None,0)
	def top_posts(self, CacheUpdated = False):
			if CacheUpdated or not self.mc_get("blog-post"):
				postQuery = db.GqlQuery("SELECT * FROM Post "
														 "ORDER BY created DESC "
														 "LIMIT 10" )
				logging.info(">>> DATABASE ACCESS <<<")
				self.mc_set("blog-post",list(postQuery))
			else: 
				logging.info(">>> CASHE HIT <<<")
			return self.mc_get("blog-post")
...
class MainPage(Handler):   
		...
		def post(self):
			...
			if u+p+v+e=="":
				...
				self.response.headers.add_header('Set-Cookie', 'user_id=%s; Path=/; Max-Age=3600' % val) # change 120 to 60 later
...
class MainBlogPage(Handler): 
	def render_blogs(self):
		...
		#array = user_id.split("|") 
		if self.top_posts()[0]:
			posts = self.top_posts()[0]
		else:
			posts = self.top_posts(CacheUpdated = True)[0]
		if not self.request.url.endswith('.json'):
			self.render("posts.html", posts=posts, time_plus=self.time_plus, timedelta=timedelta, username=self.checkCookieTampering(), age=self.age_str(self.mc_get("blog-post")[1]))
		else:
			self.render_json([post.as_dict() for post in posts])
	def get(self):
		logging.info("***** MainPage GET *****")
		self.response.headers['Content-sType']='text/html'  
		self.render_blogs()
...
class Newpost(Handler):   
	...
	def post(self):
		...
		if (subject and content):
			...
			time.sleep(0.2)
			postInst.key().id()
			self.top_posts(CacheUpdated=True)
...
class ProductHandler(Handler):   
	def get(self, id):
		...
		if self.mc_get(id) == (None,0):
			logging.info("**************** ADDING POST TO MEMCACHE ******************")
			self.mc_set(id, Post.get_by_id(int(id)))
		post = self.mc_get(id)[0]

		if not self.request.url.endswith('.json'):
			self.render("permalink.html", Postnumber=id, post=post, time_plus=self.time_plus, timedelta=timedelta, username=self.checkCookieTampering(), age=self.age_str(self.mc_get(id)[1]))
		else:
